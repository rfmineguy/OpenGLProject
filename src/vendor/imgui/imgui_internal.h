...
// "Paranoid" Debug Asserts are meant to only be enabled during specific debugging/work, otherwise would slow down the src too much.
// We currently don't have many of those so the effect is currently negligible, but onward intent to add more aggressive ones in the src.
...
IMGUI_API int           ImTextCountCharsFromUtf8(const char* in_text, const char* in_text_end);                            // return number of UTF-8 src-points (NOT bytes count)
...
// Helpers: Maths
// - Wrapper for standard libs functions. (Note that imgui_demo.cpp does _not_ use them to keep the src easy to copy)
...
enum ImGuiInputSource
{
    ...
    ImGuiInputSource_NavKeyboard,   // Only used occasionally for storage, not tested/handled by most src
...
// Simple column measurement, currently used for MenuItem() only.. This is very short-sighted/throw-away src and NOT a generic helper.
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    ImVector<char>          TextA;                  // temporary UTF8 buffer for callbacks and other operations. this is not updated in every src-path! size=capacity.
    ...
    void        OnKeyPressed(int key);      // Cannot be inline because we call in src in stb_textedit.h implementation
...
struct ImGuiContext
{
    ...
    ImGuiID                 NavFocusScopeId;                    // Identify a selection scope (selection src often wants to "clear other items" when landing on an item of the selection set)
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImDrawList*             DrawList;                           // == &DrawListInst (for backward compatibility reason with src using imgui_internal.h we keep this a pointer)
...
namespace ImGui
{
    ...
    // To use them externally (for custom widget) you may need an "extern template" statement in your src in order to link to existing instances and silence Clang warnings (see #2036).

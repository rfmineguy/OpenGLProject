...
struct ImFontGlyph;                 // A single font glyph (src point + coordinates within in ImFontAtlas + offset)
...
struct ImGuiOnceUponAFrame;         // Helper for running a block of src not more than once a frame, used by IMGUI_ONCE_UPON_A_FRAME macro
...
typedef unsigned short ImWchar16;   // A single decoded U16 character/src point. We encode them as multi bytes UTF-8 when used in strings.
typedef unsigned int ImWchar32;     // A single decoded U32 character/src point. We encode them as multi bytes UTF-8 when used in strings.
...
namespace ImGui
{
    ...
    // - Those are merely shortcut to calling Text() with a format string. Output single value in "name: value" format (tip: freely declare more in your src to handle your types. you can add functions to the ImGui namespace)
    ...
    // - To refer to a mouse button, you may use named enums in your src e.g. ImGuiMouseButton_Left, ImGuiMouseButton_Right.
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_HorizontalScrollbar    = 1 << 11,  // Allow horizontal scrollbar to appear (off by default). You may use SetNextWindowContentSize(ImVec2(width,0.0f)); prior to calling Begin() to specify width. Read src in imgui_demo in the "Horizontal Scrolling" section.
    ...
    ImGuiWindowFlags_UnsavedDocument        = 1 << 20,  // Append '*' to title without affecting the ID, as a convenience to avoid using the ### operator. When used in a tab/docking context, tab is selected on closure and closure is deferred by one frame to allow src to cancel the closure (with a confirmation popup, etc.) without flicker.
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CallbackAlways      = 1 << 8,   // Callback on each iteration. User src may query cursor position, modify text buffer.
...
enum ImGuiTabItemFlags_
{
    ...
    ImGuiTabItemFlags_UnsavedDocument               = 1 << 0,   // Append '*' to title without affecting the ID, as a convenience to avoid using the ### operator. Also: tab is selected on closure and closure is deferred by one frame to allow src to undo it without flicker.
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_SourceNoDisableHover         = 1 << 1,   // By default, when dragging we clear data so that IsItemHovered() will return false, to avoid subsequent user src submitting tooltips. This flag disable this behavior so you can still call IsItemHovered() on the source item.
...
#define IMGUI_PAYLOAD_TYPE_COLOR_3F     "_COL3F"    // float[3]: Standard type for colors, without alpha. User src may use this type.
#define IMGUI_PAYLOAD_TYPE_COLOR_4F     "_COL4F"    // float[4]: Standard type for colors. User src may use this type.
...
enum ImGuiConfigFlags_
{
    ...
    // User storage (to allow your back-end/engine to communicate to src that may be shared between multiple projects. Those flags are not used by core Dear ImGui)
...
// - The enum only refers to fields of ImGuiStyle which makes sense to be pushed/popped inside UI src.
...
// User src may request binding to display given cursor by calling SetMouseCursor(), which is why we have some cursors that are marked unused here
...
//-----------------------------------------------------------------------------
// Helper: ImVector<>
// Lightweight std::vector<>-like class to avoid dragging dependencies (also, some implementations of STL with debug enabled are absurdly slow, we bypass it so our src runs fast in debug).
//-----------------------------------------------------------------------------
// - You generally do NOT need to care or use this ever. But we need to make it available in imgui.h because some of our public structures are relying on it.
// - We use std-like naming convention here, which is a little unusual for this codebase.
// - Important: clear() frees memory, resize(0) keep the allocated buffer. We use resize(0) a lot to intentionally recycle allocated buffers across frames and amortize our costs.
// - Important: our implementation does NOT call C++ constructors/destructors, we treat everything as raw data! This is intentional but be extra mindful of that,
//   Do NOT use this class as a std::vector replacement in your own src! Many of the structures used by dear imgui can be safely initialized by a zero-memset.
//-----------------------------------------------------------------------------
...
#define IM_UNICODE_CODEPOINT_INVALID 0xFFFD     // Invalid Unicode src point (standard value).
#ifdef IMGUI_USE_WCHAR32
#define IM_UNICODE_CODEPOINT_MAX     0x10FFFF   // Maximum Unicode src point supported by this build.
#else
#define IM_UNICODE_CODEPOINT_MAX     0xFFFF     // Maximum Unicode src point supported by this build.
#endif

// Helper: Execute a block of src at maximum once a frame. Convenient if you want to quickly create an UI within deep-nested src that runs multiple times every frame.
...
// - You want to store custom debug data easily without adding or editing structures in your src (probably not efficient, but convenient)
...
// - User src submit one element.
// - Clipper can measure the height of the first element
// - Clipper calculate the actual range of elements to display based on the current clipping rectangle, position the cursor before the first visible element.
// - User src submit visible elements.
...
struct ImDrawCmd
{
    ...
    void*           UserCallbackData;   // 4-8  // The draw callback src can access this.
...
// The src expect ImVec2 pos (8 bytes), ImVec2 uv (8 bytes), ImU32 col (4 bytes), but you can re-order them or add other fields as needed to simplify integration in your engine.
...
{
    ImVector<ImU32> UsedChars;            // Store 1-bit per Unicode src point (0=unused, 1=used)
...
// It is the user-src responsibility to setup/build the atlas, then upload the pixel data into a texture accessible by your graphics api.
//  - Optionally, call any of the AddFont*** functions. If you don't call any, the default font embedded in the src will be loaded for you.
...
struct ImFontAtlas
{
    ...
    // NB: Make sure that your string are UTF-8 and NOT in your local src page. In C++11, you can create UTF-8 string literal using the u8"Hello world" syntax. See FAQ for details.
...
struct ImFont
{
    ...
    ImVector<ImWchar>           IndexLookup;        // 12-16 // out //            // Sparse. Index glyphs by Unicode src-point.

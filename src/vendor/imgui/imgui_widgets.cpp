...
// The entire piece of src below is rather confusing because:
// - We handle absolute seeking (when first clicking outside the grab) and relative manipulation (afterward or when clicking inside the grab)
// - We store values as normalized ratio and in a form that allows the window content to change while we are holding on a scrollbar
// - We handle both horizontal and vertical scrollbars, which makes the terminology not ideal.
// Still, the src should probably be made simpler..
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    // Handle input right away. None of the src of Begin() is relying on scrolling position before calling Scrollbar().
    bool held = false;
    bool hovered = false;
...
// Read src of e.g. DragFloat(), DragInt() etc. or examples in 'Demo->Widgets->Data Types' to understand how to use this function directly.
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    else
    {
        ...
        else
        {
            ...
            //   This src is carefully tuned to work with large values (e.g. high ranges of U64) while preserving this property..
...
// FIXME: Move more of the src into SliderBehavior()
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
...
// Read src of e.g. SliderFloat(), SliderInt() etc. or examples in 'Demo->Widgets->Data Types' to understand how to use this function directly.
...
// FIXME: This is still used by some navigation src path to infer a minimum tweak step, but we should aim to rework widgets so it isn't needed.
...
// However this may not be ideal for all uses, as some user src may break on out of bound values.
...
// Read src of e.g. InputFloat(), InputInt() etc. or examples in 'Demo->Widgets->Data Types' to understand how to use this function directly.
...
// FIXME: The existence of this rarely exercised src path is a bit of a nuisance.
...
void ImGuiInputTextCallbackData::InsertChars(int pos, const char* new_text, const char* new_text_end)
{
    ...
    if (new_text_len + BufTextLen >= BufSize)
    {
        ...
        // Contrary to STB_TEXTEDIT_INSERTCHARS() this is working in the UTF8 buffer, hence the mildly similar src (until we remove the U16 buffer altogether!)
        ImGuiContext& g = *GImGui;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    // FIXME-OPT: Because our selection/cursor src currently needs the wide text we need to convert it when active, which is not ideal :(
    ...
    if (g.ActiveId == id)
    {
        ...
        // If we didn't do that, src like InputInt() with ImGuiInputTextFlags_EnterReturnsTrue would fail.
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            ImU32 bg_color = GetColorU32(ImGuiCol_TextSelectedBg, render_cursor ? 1.0f : 0.6f); // FIXME: current src flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
...
// (In C++ the 'float col[4]' notation for a function argument is equivalent to 'float* col', we only specify a size to facilitate understanding of the src.)
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
            if (ColorEdit4("##rgb", col, sub_flags | ImGuiColorEditFlags_DisplayRGB))
            {
                ...
                // For the later we don't want to run the hue-wrap canceling src. If you are well versed in HSV picker please provide your input! (See #2050)
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    {
        // Because GetColorU32() multiplies by the global style Alpha and we don't want to display a checkerboard if the source src had no alpha
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    // allow browsing a tree while preserving selection with src implementing multi-selection patterns.
...
bool ImGui::ListBox(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
    ...
    // Assume all items have even height (= 1 line of text). If you need items of different or variable sizes you can create a custom version of ListBox() in your src without using the clipper.
    ImGuiContext& g = *GImGui;
    bool value_changed = false;
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if (!enabled) // explicitly close if an open menu becomes disabled, facilitate users src a lot in pattern such as 'if (BeginMenu("options", has_object)) { ..use object.. }'
        want_close = true;
...
void ImGui::EndMenu()
{
    ...
    // However, it means that with the current src, a BeginMenu() from outside another menu or a menu-bar won't be closable with the Left direction.
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
static void ImGui::TabBarScrollToTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab, ImGuiTabBarSection* sections)
{
    ...
    // We make all tabs positions all relative Sections[0].Width to make src simpler
...
void ImGui::NextColumn()
{
    ...
    // FIXME-COLUMNS: Share src with BeginColumns() - move src on columns setup.
    float offset_0 = GetColumnOffset(columns->Current);
    float offset_1 = GetColumnOffset(columns->Current + 1);
    float width = offset_1 - offset_0;
    PushItemWidth(width * 0.65f);

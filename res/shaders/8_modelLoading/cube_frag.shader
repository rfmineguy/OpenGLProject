...
void main()
{
    ...
    vec3 result = CalcDirLight(environment.dirLight, norm, viewDir);

    //calculate point lighting
    for(int i = 0; i < NR_POINT_LIGHTS; i++) {
        result += CalcPointLight(environment.pointLights[i], norm, FragPos, viewDir);
    }

    result += CalcSpotLight(environment.spotLight, norm, FragPos, viewDir);
    float alpha = 1.0;
